...
import $ from 'jquery'
import {  Gateway } from '/@/api/backend/debug/gateway'
...
var GatewayTextMap=new Map()
var AreaMap=new Map()
var TagMap=new Map()
export function loadSVGData (data) {
    GatewayMap.clear()
    GatewayTextMap.clear()
    TagMap.clear()
    //外部容器的高度和宽度
    //console.log(scale);
    var div = $("#main");
...
export function clear(address:string){

    var text = GatewayTextMap.get(address)
    if(text!=null){
       text.text('')
    }
}
export function updateGateway(gateway:Gateway) {
     console.log("开始更新"+gateway.address)
    if (draw == null) {
        return;
    }
    ...
    console.log("开始更新"+gateway.last_time)
    group.data("data", {
        value: gateway
    });
    console.log("保存数据")
    ...
    var image
    console.log("保存数据66"+group.data().data.value.online)
 //   if(gateway.online==group.data().data.value.online){
     //   group.clear()
        if(gateway.online==1&&!gateway.is_use){
            console.log("网关在线")
            image= group.image("/src/assets/gateway_online.png")
          //  group.text('')
            image.size(5,5);
            image.move(gateway.x * gateway.proportion , gateway.y * gateway.proportion );
        }else if(gateway.online==1&&gateway.is_use){
            console.log("使用这里")
            image= group.image("/src/assets/gateway_online.png")
            var text = GatewayTextMap.get(gateway.address)
            if(text==null){
                 text= group.text(gateway.rssi).font({
                    family:   'Helvetica'
                    , size:     3
                    , anchor:   'middle'
                    , leading:  '1.0em'
                }).move(gateway.x * gateway.proportion, gateway.y * gateway.proportion -(imgWidth));
            }else{
              text.text(gateway.rssi)
                text.font({
                    family:   'Helvetica'
                    , size:     3
                    , anchor:   'middle'
                ...
                text.move(gateway.x * gateway.proportion, gateway.y * gateway.proportion -(imgWidth));
            }

            GatewayTextMap.set(gateway.address,text)
       //     console.log("文本长度="+GatewayTextMap.size)
            image.size(5,5);
            image.move(gateway.x * gateway.proportion, gateway.y * gateway.proportion );
        }
        else{
...
export function addTag(tag:Tag) {
   ...
   //
    ...
    if(group==null){
      ...
      // var text= group.text(tag.name)
         //  text.font({ fill: '#000', family: 'Inconsolata' , size: 1})
       //   text .move(tag.x * tag.proportion - (imgWidth) ,  tag.y*7 ).font({ fill: '#000', family: 'Inconsolata' , size: 1})
