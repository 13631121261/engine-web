...
export function loadSVG(callback: (data,point,svg) => void,svg:string){
    draw =  SVG('#svgPanel');
    traces=[];
     dblFlag = false;
     mouseMoveFlag = true;
    ...
    $("#svgPanel").on("click", function(e) {
        ...
        if (traces.length > 0) {
            ...
            console.log("-----moveTo----"+lastPoint.x+"--"+ lastPoint.y);
            ctx.lineTo(e.offsetX, e.offsetY);

            console.log("-----lineTo----"+e.offsetX+"--"+  e.offsetY);

            movetoArr.push(lastPoint.x+","+lastPoint.y+";");
            linetoArr.push(e.offsetX+","+ e.offsetY+";");
        ...
        if (traces.length > 3) {
            ...
            createPolyline(str);
             polygon = draw.polygon(str)
            polygon.opacity(0.1)
            polygon.fill('#0aa')

            mouseMoveFlag = false;
            callback(str,traces, svg);
