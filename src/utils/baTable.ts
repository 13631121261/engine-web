...
export default class baTable {
    ...
    constructor(api: baTableApi, table: {
        dblClickNotEditColumn: (undefined | string)[]; column: ({ type: string; align: string; operator: boolean } | { operatorPlaceholder: string; prop: string; width: number; fixed: string; label: string; align: string; operator: string } | { operatorPlaceholder: string; prop: string; width: number; label: string; align: string; operator: string } | { operatorPlaceholder: string; prop: string; width: number; label: string; align: string; operator: string } | { prop: string; width: number; label: string; align: string; render: string } | { prop: string; width: number; label: string; align: string; render: string } | { prop: string; width: number; label: string; align: string } | { prop: string; width: string; label: string; align: string } | { prop: string; width: string; label: string; align: string } | { prop: string; width: string; label: string; align: string; render: string } | { buttons: OptButton[]; width: string; fixed: string; label: string; align: string; render: string; operator: boolean })[]; expandAll: boolean
    }, form: BaTableForm = {}, before: BaTableBefore = {}, after: BaTableAfter = {}) {
        this.api = api
    ...
    // 导出
    export = () => {
        if (this.runBefore('export', {  }) === false) return
        this.api.export().then((res) => {
            let blob = res         // 接收到的数据
                  let eleLink = document.createElement('a')   // 创建 a标签
                  let url = URL.createObjectURL(blob)   // 创建url对象 指向blob
                  eleLink.href = url        // 设置a标签的href属性
                  eleLink.download = `文件名字.xlsx`          // 下载的文件名 后缀名与后台约定
                  document.body.appendChild(eleLink)   // 将a标签添加到文档中
                  eleLink.click()    //执行点击事件 开始下载
                  window.URL.revokeObjectURL(url)    // 释放指定的url
            console.log("下载文件")
        })
    }
    // 编辑
    ...
    toggleForm = (operate = '', operateIds: string[] = []) => {
        ...
        if (operate == 'Edit') {
            ...
            console.log(this.form.defaultItems)
        ...
        else if(operate=='OutPut'){
            this.form.items = cloneDeep(this.form.defaultItems)
            console.log("OutPut")
            console.log(  this.form.defaultItems)
            console.log(  this.form.items)
        }else if(operate=='bind'||operate=='unbind'||operate=='return'){
            this.form.items = cloneDeep(this.form.defaultItems)
        }
        if (operate == 'update') {
            if (!operateIds.length) {
            ...
            console.log(operateIds)
           // this.requestEdit(operateIds[0])
        }
        console.log("operate="+operate)
        console.log("operateIds="+operateIds)
    ...
    onSubmit = (formEl: FormInstance | undefined = undefined) => {
        ...
        const submitCallback = () => {
            if(operate=='update'){
                ...
                this.form.items.ids=    this.form.operateIds
                this.api
    ...
    onTableHeaderAction = (event: string, data: anyObj) => {
        ...
        const actionFun = new Map([
            ...
            [
                'update',
                () => {
                    this.toggleForm('update', this.getSelectionIds())
                },
            ],
            [
                'delete',
            ...
            [
                'alarm_object',
                () => {
                    this.table.filter!.alarm_object = data.keyword
                    this.getIndex()
                },
            ],
            [
                'alarm_type',
                () => {
                    this.table.filter!.alarm_type = data.keyword
                    this.getIndex()
                },
            ],
            [
                'bind_status',
                () => {
                    this.table.filter!.bind_status = data.keyword
                    this.getIndex()
                },
            ],
            [
                'history',
                () => {
                    this.table.filter!.history_type = data.history_type
