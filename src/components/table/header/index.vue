<template>
    ...
    <div v-bind="$attrs" class="table-header ba-scroll-style">
        ...
        <el-tooltip v-if="props.buttons.includes('update') && auth('update')" :content="t('update')" placement="top">
            <el-button v-blur @click="onAction('update')" :disabled="!enableBatchOpt" class="table-header-operate" type="primary">
                <Icon name="fa fa-cloud-upload" />
                <span class="table-header-operate-text">{{ t('update') }}</span>
            </el-button>
        </el-tooltip>
<!--        <el-tooltip v-if="props.buttons.includes('update') && auth('update')" :content="t('update')" placement="top">
            <el-button v-blur @click="onAction('update')" class="table-header-operate" type="primary">
                <Icon name="fa fa-plus" />
                <span class="table-header-operate-text">{{ t('update') }}</span>
            </el-button>
        </el-tooltip>-->
        <el-tooltip v-if="props.buttons.includes('import') && auth('import')" :content="t('Import from Excel')" placement="top">
            ...
            <el-upload
                ...
                name='data'
                auto-upload='ture'
                :data='useAdminInfo().getProject_Key()'
                ons
            >
            <el-button v-blur  class="table-header-operate" type="primary" style='margin-left: 10px'>
                <Icon name="fa fa-pencil" />
        ...
        <el-tooltip v-if="props.buttons.includes('export') && auth('export')" :content="t('export_tip')" placement="top">
            <el-button v-blur @click="onAction('export')"  class="table-header-operate" type="primary" style='margin-left: 10px'>
                <Icon name="fa fa-pencil" />
...
const onSuccess=(response: any, uploadFile: UploadFile, uploadFiles: UploadFiles) => {
    ...
    if(response.code==1){
        if(response.data.length>0){
            var name=response.data.replaceAll("\\","5678").replaceAll(":","8765");
            var ip= "http://"+window.location.host+"/userApi/downResult?file_name="+name
...
function onHeader() {
    console.log("aassd")
    return {"Batoken": useAdminInfo().getToken()}
}
