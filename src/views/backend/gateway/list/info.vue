        ...
        <el-scrollbar v-loading="baTable.form.loading" class="ba-table-form-scrollbar">
                ...
                <div class="demo-collapse">
                    <el-collapse v-model="activeName" accordion>
                        <el-collapse-item :title="t('Network')" name="1">

                            <div class="container1">
                                <div class='item1'>
                                    <div  class="text item" >
                                        <span>{{t('gateway_ble_version')}}</span>
                                        {{': '+baTable.form.items!.ble_version  }}</div>
                                    <div  class="text item" >
                                        <span >{{t('gateway_wifi_version')}}</span>
                                        {{': '+baTable.form.items!.wifi_version  }}</div>

                                    <div  class="text item" >
                                        <span>{{t('run_time')}}</span>
                                    ...
                                    <div  class="text item" >
                                        <span>{{t('gateway_wifi_status')}}</span>
                                        <el-tag v-if="baTable.form.items!.wifi_onoff=='on'">{{t('open')}}</el-tag>
                                        <el-tag v-else>{{t('close')}}</el-tag>
                                    </div>
                                    <div  class="text item" >
                                        <span>{{t('sub_topic')}}</span>
                                        {{': '+baTable.form.items!.sub_topic  }}</div>
                                </div>
                                <div class='item1'>
                                    <div  class="text item" >
                                        <span>{{t('syn_fun')}}</span>
                                        <el-tag v-if='baTable.form.items!.isyn==1'>{{t('open')}}</el-tag>
                                        <el-tag v-else>{{t('close')}}</el-tag>


                                    </div>
                                    <div  class="text item" >
                                        <span>{{t('gateway_lo_ipv4')}}</span>
                                        {{': '+baTable.form.items!.wlan_ip  }}</div>
                                    <div  class="text item" >
                                        <span >{{t('mode')}}</span>
                                        {{': '+baTable.form.items!.op_mode  }}</div>
                                    <div  class="text item" >
                                        <span >{{t('dhcp_static')}}</span>
                                        {{': '+baTable.form.items!.wan_mode  }}</div>
                                    <div  class="text item" >
                                        <span >{{t('gateway_ipv4')}}</span>
                                        {{': '+baTable.form.items!.wan_ip  }}</div>
                                    <div  class="text item" >
                                        <span >{{t('route_gateway')}}</span>
                                        {{': '+baTable.form.items!.wan_gw  }}</div>
                                    <div  class="text item" >
                                        <span>{{t('online_time')}}</span>
                                        {{': '+dateFormat(baTable.form.items!.lasttime)  }}</div>
                                <div  class="text item" >
                                    <span>{{t('pub_topic')}}</span>
                                    {{': '+baTable.form.items!.pub_topic  }}</div>
                            </div>
                            </div>

                        </el-collapse-item>
                        <el-collapse-item :title="t('report_param')" name="2">
                            <div class="container1">
                                <div class='item1'>
                                    <div  class="text item" >
                                        <span>{{t('scan_out')}}：</span>
                                        <el-tag v-if='baTable.form.items!.scan_out==0'>{{t('close')}}</el-tag>
                                        <el-tag v-else>{{t('open')}}</el-tag>
                                    </div>
                                    <div  class="text item" >
                                        <span>{{t('report_interval')}}</span>
                                        {{': '+baTable.form.items!.scan_interval  +'ms'}}</div>

                                    <div  class="text item" >
                                        <span>{{t('report_type')}}：</span>
                                        <el-tag v-if='baTable.form.items!.report_type==1'>{{t('adv_only')}}</el-tag>
                                        <el-tag v-else-if='baTable.form.items!.report_type==2'>{{t('Scan_Response')}}</el-tag>
                                        <el-tag v-else-if='baTable.form.items!.report_type==3'>{{t('OnlyMac_Rssi')}}</el-tag>
                                        <el-tag v-else-if='baTable.form.items!.report_type==4'>{{t('Ble_Work_card')}}</el-tag>
                        ...
                        <el-collapse-item :title="t('Scan_filtering')" name="3">
                            <div class="container1">
                                <div class='item1'>
                                        <div  class="text item" >
                                            <span>{{t('rssi_filter')}}</span>
                                            <el-tag v-if='baTable.form.items!.filter_rssi <0'>{{+baTable.form.items!.filter_rssi  }}</el-tag>
                                            <el-tag v-else>{{t('close')}}</el-tag>
                                        </div>
                                        <div  class="text item" >
                                            <span>{{t('name_filter')}}</span>
                                        ...
                                        <div  class="text item" >
                                            <span>{{t('beacon_filter')}}</span>
                                            <el-tag v-if='baTable.form.items!.filter_ibeacon===0'>{{t('close')}}</el-tag>
                                            &nbsp;
                                            <el-tag v-else>{{t('open')}}</el-tag>
                                        </div>
                                        <div  class="text item" >
                                            <span>{{t('mac_filter')}}：</span>
