...
declare global {
    ...
    interface TableColumn extends Partial<TableColumnCtx<TableRow>> {
        ...
        // 渲染为:icon|switch|image|images|beacon|url|datetime|buttons|customTemplate|customRender|slot
        render?:
            | 'icon'
            | 'switch'
            | 'image'
        ...
        // 操作按钮组
        buttons?: OptButton[]
        // 渲染为Tag时:el-beacon 组件的主题
        effect?: TagProps['effect']
        // 渲染为Tag时:el-beacon 组件的size
        size?: TagProps['size']
        ...
        // 公共搜索渲染方式:上方的 render=beacon|switch 时公共搜索也会渲染为下拉，数字会渲染为范围筛选，时间渲染为时间选择器等
